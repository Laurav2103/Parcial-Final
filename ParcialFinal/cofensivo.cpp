...
cOfensivo::cOfensivo(float r,float Ho,float D, float Hd,QGraphicsItem *parent): QGraphicsItem(parent)
{
    posy=Ho;
    Xd=D;
    Yd=Hd;
    ra=r;
    sprite = QPixmap(":/balaa (1).png");
    posSprite=0;
//    bulletOf=new QGraphicsEllipseItem;
//    bulletOf->setRect(0,10,20,20);
//    bulletOf->setBrush(Qt::black);
   // scene->addItem(bulletOf);
    disparosOf(Xd,Yd);
